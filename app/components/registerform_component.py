...
from app.controllers import AuthController, UsersController
...
@reactive_form(
    ...
    validation_rules = {
        # 'first_name': 'validate_username',
        # 'last_name': 'validate_username',
        # 'email': 'validate_email',
        'password': [
            FormFieldValidationRule(
                validate_fn = lambda value: not len(value) < 8,
    ...
    on_submit = 'perform_register',  # lambda form: print(f"Submitted Form: {form.get_values()}"),
    on_submit_failed = 'process_errors',
    auto_validate = False
)
class Registerform(Column):    # 🧩 You can replace ft.Container with any other Flet control
    """Reactive Registerform Widget"""
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Reactive Fields Properties
        self.rx_first_name = RxStr("")
        self.rx_last_name = RxStr("")
        self.rx_phone = RxStr("")
        self.rx_password = RxStr("")
        self.rx_role = RxStr("")
        self.rx_avatar = RxStr("")
        self.rx_accepted = RxBool(False)
        ...
        # self.userController: UsersController = FletX.find(
    ...
    def perform_register(self, form: Any):
        ...
        try:
            ...
            if form.is_valid:
                ...
                res = self.authController.register(
                    UserInfo(
                        first_name = from_values.get('username', ''),
                        last_name = from_values.get('username', ''),
                ...
                if not res:
                    ...
                    self.formatted_errors_msg.value = self.authController._error_message.value
                    return
                
                print(f"User Created: {res.to_json()}")
            ...
            self.process_errors({"":str(e)})

    def build(self):
        """Build the Registerform content"""
        ...
        return [
            ...
            Row(
                ...
                controls = [
                    Obx(
                        builder_fn = lambda: Checkbox(
                            value = self.rx_is_valid.value,
                            label = Text(
                                "I agree to the Python Togo ",
                                size = 13,
                                color = Colors.ON_SURFACE_VARIANT
                            ),
            ...
            GestureDetector(
                on_tap = lambda _: navigate('/login'),
                content = Row(
                    expand = True,
                    spacing = 2,
                    alignment = MainAxisAlignment.CENTER,
